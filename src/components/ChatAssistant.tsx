...
const ChatAssistant: React.FC<ChatAssistantProps> = ({ isFloating = false }) => {
  const { user } = useAuth();
  const [messages, setMessages] = useState<Message[]>([
    {
      id: '1',
      content: "Bonjour ! Je suis votre assistant de documents. Posez-moi des questions sur vos projets et documents.",
      sender: 'assistant',
      timestamp: new Date(),
    },
  ]);
  ...
  return (
    <div className={containerClasses}>
        ...
        <div className={`flex justify-between items-center ${headerClasses}`}>
          <div className="flex items-center gap-2">
            <Avatar className="h-8 w-8 bg-primary/10">
              <AvatarFallback><BotIcon className="h-4 w-4 text-primary" /></AvatarFallback>
            </Avatar>
      ...
      <div className={isFloating ? "flex-1 overflow-hidden" : "flex-1 mb-4 overflow-hidden"}>
        <div className={`${isFloating ? 'h-[calc(100%-40px)]' : 'h-full bg-card rounded-lg border shadow-sm'} overflow-y-auto p-3`}>
                ...
                >
                  {message.sender === 'assistant' && (
                    <Avatar className="h-6 w-6 mr-1 mt-1">
                      <AvatarFallback><BotIcon className="h-3 w-3" /></AvatarFallback>
                    </Avatar>
                  )}
      ...
      {isFloating && (
        <Button variant="ghost" size="sm" className="self-end mb-1 h-7 px-2 text-xs" onClick={clearChat}>
          <TrashIcon className="h-3 w-3 mr-1" />
          Effacer
        </Button>
      )}
      
      <form onSubmit={handleSendMessage} className={`flex gap-2 ${isFloating ? 'px-3 pb-3' : ''}`}>
        <Input
