...
const DocumentCard: React.FC<DocumentCardProps> = ({ 
  ...
  const fileSizeFormatted = document.fileSizeFormatted || 
    ...
    <Card className="bg-card text-card-foreground shadow-sm">
      <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
        ...
        <div className="space-x-1">
          <Button variant="ghost" size="icon" onClick={() => onToggleStar(document)}>
            <StarIcon className={`h-4 w-4 ${document.isStarred ? 'text-yellow-500' : 'text-muted-foreground'}`} />
          </Button>
        </div>
      </CardHeader>
      ...
      <CardContent className="grid gap-4 py-2 text-sm">
        ...
        <div className="flex items-center">
          ...
          <div className="flex items-center ml-1">
            {getAccessLevelIcon()}
            <span>{accessLevelTranslations[document.accessLevel as keyof typeof accessLevelTranslations] || document.accessLevel}</span>
          </div>
        </div>
      ...
      <CardFooter className="flex justify-between items-center py-2">
        ...
        <div className="space-x-1">
          <Button variant="outline" size="icon" onClick={() => onView(document)}>
            <EyeIcon className="h-4 w-4" />
          </Button>
          <Button variant="outline" size="icon" onClick={() => onEdit(document)}>
          ...
          <Button variant="destructive" size="icon" onClick={() => onDelete(document)}>
